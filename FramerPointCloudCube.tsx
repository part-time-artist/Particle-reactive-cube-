...
function PointCloud({ color, pointCount, pointSize, speed, boxSize, gravityRadius, gravityStrength }) {
    ...
    const particles = React.useMemo(() => {
        ...
        for (let i = 0; i < pointCount; i++) {
            // SURFACE DISTRIBUTION LOGIC
            // 1. Pick a random face (0 to 5)
            const face = Math.floor(Math.random() * 6)
            // 2. Randomize two coordinates, fix the third to the edge
            ...
            switch (face) {
                case 0: x = half; y = u; z = v; break; // Right
                case 1: x = -half; y = u; z = v; break; // Left
                case 2: x = u; y = half; z = v; break; // Top
                case 3: x = u; y = -half; z = v; break; // Bottom
                case 4: x = u; y = v; z = half; break; // Front
                case 5: x = u; y = v; z = -half; break; // Back
                default: x = 0; y = 0; z = 0;
            }
...
export default function PointCloudCube(props) {
    const {
        color, pointCount, pointSize, speed, boxSize,
        gravityRadius, gravityStrength, enableRotate, backgroundColor,
        fogNear, fogFar
    } = props

    return (
        <div style={{ width: "100%", height: "100%", overflow: "hidden", background: backgroundColor }}>
            <Canvas camera={{ position: [0, 0, 6], fov: 60 }} raycaster={{ params: { Points: { threshold: 0.2 } } }}>
...
addPropertyControls(PointCloudCube, {
    color: { type: ControlType.Color, title: "Color", defaultValue: "#0099ff" },
    backgroundColor: { type: ControlType.Color, title: "Background", defaultValue: "#050505" },
    pointCount: { type: ControlType.Number, title: "Points", defaultValue: 12000, min: 1000, max: 30000, step: 1000 },
    pointSize: { type: ControlType.Number, title: "Size", defaultValue: 0.025, min: 0.005, max: 0.2, step: 0.005 },
    speed: { type: ControlType.Number, title: "Rot Speed", defaultValue: 0.2, min: 0, max: 2, step: 0.1 },
    boxSize: { type: ControlType.Number, title: "Box Size", defaultValue: 4, min: 1, max: 10 },
    gravityRadius: { type: ControlType.Number, title: "Gravity Radius", defaultValue: 2.5, min: 0.1, max: 5 },
    gravityStrength: { type: ControlType.Number, title: "Gravity Force", defaultValue: 0.2, min: 0, max: 10, step: 0.1 },
    fogNear: { type: ControlType.Number, title: "Fog Near", defaultValue: 4, min: 0, max: 20 },
    fogFar: { type: ControlType.Number, title: "Fog Far", defaultValue: 8, min: 0, max: 20 },
    enableRotate: { type: ControlType.Boolean, title: "Rotate", defaultValue: true },
})
